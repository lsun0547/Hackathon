...
moon_options = ['New Moon', 'Young Moon', 'Waxing Crescent', 'Waxing Quarter', 'Waxing Gibbous', 'Full Moon', 'Waning Gibbous', 'Waning Quarter', 'Waning Crescent', 'Old Moon']
...
def generate_entry():
    ...
    if (weather in ['Foggy', 'Stormy'] or int(time.split(':')[0]) in range(0, 5)
        ...
        haunted = random.choices([1, 0], weights=[0.4, 0.6])[0]
    elif (moon in ['Full Moon'] and int(time.split(':')[0]) in range(20, 5) or (location in ['Grave 42'] and day in ['Friday']) or (temp in ['Very Cold'] and weather in ['Foggy', 'Stormy'])):
        haunted = random.choices([1, 0], weights=[0.7, 0.3])[0]
    else:
